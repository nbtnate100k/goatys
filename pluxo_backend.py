...
def _document_default_base_name() -> str:
    raw = (os.environ.get("PLUXO_DOCUMENT_DEFAULT_BASE", "MONEY_BASE") or "").strip().upper()
    return raw if ADMIN_STOCK_BASE_NAME_RE.fullmatch(raw) else "MONEY_BASE"
...
def parse_stock_inventory_document(
    ...
    if current_base is None:
        for cand in ("MONEY_BASE", "TONY_BASE"):
        ...
        if re.match(r"^\d{6,19}\|", st):
            if not current_base:
                return (
                    [],
                    None,
                    "Card line appears before a valid base (add a header like MONEY_BASE or REY_BASE, "
...
async def tg_stock(update, context) -> None:
    ...
    await msg.reply_text(
        ...
        f"🌐 Same data as the shop — <b>refresh the site</b>. If the site stays empty, its "
        f"<code>pluxo_api_base</code> / Railway URL must point to <b>this</b> server.",
        parse_mode="HTML",
    )
